...
```text
EA1-IaC1/
├── environment/                # Infraestructura Terraform desplegada en AWS
│   ├── provider.tf
│   ├── network.tf
│   ├── ec2.tf
│   ├── rds.tf
...
│   └── README.md
├── Evidence/                   # Evidencias de ejecución, pantallas y validaciones
├── .gitignore
├── README.md
└── terraform.tfstate*          # generado al ejecutar Terraform (si aplica)
```

## Instrucciones Generales

Siga esta secuencia para completar la entrega del proyecto:

1. Configure las credenciales de AWS y valide el acceso al entorno AWS Academy.
2. Ubique la infraestructura en la carpeta `environment/` y mantenga la configuración Terraform separada de la documentación.
3. Cree o ajuste los archivos `.tf` necesarios para la VPC, EC2, RDS y S3.
4. Ejecute `terraform init`, `terraform plan` y `terraform apply` para desplegar la infraestructura.
5. Verifique la disponibilidad del sitio web, la base de datos y el bucket S3.
6. Registre las evidencias en la carpeta `Evidence/` y documente el proceso en `doc/`.
7. Revise y confirme que la infraestructura final cumpla con los requisitos de la evaluación.
